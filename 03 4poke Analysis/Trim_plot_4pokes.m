...
function Trim_plot_4pokes (norm_data,Tim_win_data, wh_tit,fig_name1,layer,stats,a,b,ons,start_value,end_value,c,d)

% Initializing few matrices
colors = [0.4660 0.6740 0.1880; 0.4940 0.1840 0.5560; 0 0.4470 0.7410; 0.6350 0.0780 0.1840];
time_lfp = linspace(-0.5,1,size(norm_data(1001:4000),2)); % linearly spaced time samples from -0.5 to 1s
colors1 = [0.6350 0.0780 0.1840; 0 0.4470 0.7410; 0.4940 0.1840 0.5560; 0.4660 0.6740 0.1880];
...
% Legends
legend('','','','\color[rgb]{0.6350 0.0780 0.1840} First pokes with reward','','','','\color[rgb]{0 0.4470 0.7410} First pokes without reward','','','', ...
    '\color[rgb]{0.4940 0.1840 0.5560} Last rewarded poke','','','','\color[rgb]{0.4660 0.6740 0.1880} Last pokes without reward','FontSize', 12)
legend('boxoff')
...
fig_title = sprintf('All pokes_%s: %s: %s',wh_tit,fig_name1,layer); % generating the figure title
...
xlabel(tl,'Pokes','FontSize',18,'FontWeight','bold','FontName','Arial')
ylabel(tl,'AUC','FontSize',18,'FontWeight','bold','FontName','Arial')
fig_grp_tit = sprintf('Poke analysis: %s: %s: %s',wh_tit,fig_name1,layer);
